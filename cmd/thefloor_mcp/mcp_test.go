...
func TestDiscoveryMissingClassifiesOfficeNotRunning(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "not running") {
		t.Fatalf("discovery() error = %v, want office-not-running", err)
	}
}
...
func TestLivePathUsesBearerAndRoute(t *testing.T) {
	...
	if isErr || text != "# Approved" {
		t.Fatalf("call = (%q, %v)", text, isErr)
	}
	if gotAuth != "Bearer secret" || gotPath != control.RoutePlan {
...
func TestDiskFallbackAndSearch(t *testing.T) {
	...
	if bad || !strings.Contains(approved, "On-disk snapshot") || !strings.Contains(approved, "# Offline plan") {
		t.Fatalf("approved = (%q, %v)", approved, bad)
	}
	search, bad := o.call("transcript_search", json.RawMessage(`{"query":"durable"}`))
	if bad || !strings.Contains(search, "On-disk snapshot") || !strings.Contains(search, "durable plan") {
		t.Fatalf("search = (%q, %v)", search, bad)
	}
}
...
func TestWriteRefusesWhenOfficeAbsent(t *testing.T) {
	...
	if !bad || !strings.Contains(text, "start theboringfloor") {
		t.Fatalf("call = (%q, %v)", text, bad)
	}
}

func TestLimitsAndEmptyInput(t *testing.T) {
	if bounded(0, 50, 500) != 50 || bounded(999, 50, 500) != 500 || bounded(5, 50, 500) != 5 {
		t.Fatal("limit clamping failed")
	}
	o := testOffice(t, t.TempDir())
	for _, tc := range []struct{ name, args string }{{"plan_present", `{"text":"  "}`}, {"transcript_search", `{"query":" "}`}} {
		text, bad := o.call(tc.name, json.RawMessage(tc.args))
		if !bad || !strings.Contains(text, "must not be empty") {
			t.Fatalf("%s = (%q, %v)", tc.name, text, bad)
		}
	}
}
...
func TestStdioInitializeAndToolsListOnlyWritesProtocolJSON(t *testing.T) {
	...
	if err := serve(in, &out, testOffice(t, t.TempDir())); err != nil {
		t.Fatal(err)
	}
	lines := strings.Split(strings.TrimSpace(out.String()), "\n")
	if len(lines) != 2 {
		t.Fatalf("stdout protocol frames = %q", out.String())
	}
	var listed struct {
		Result struct {
			Tools []struct {
	...
	want := []string{"plan_present", "plan_update", "plan_get_approved", "transcript_read", "transcript_search", "office_status"}
	if len(listed.Result.Tools) != len(want) {
		t.Fatalf("tools = %#v", listed.Result.Tools)
	}
	for i, n := range want {
		if listed.Result.Tools[i].Name != n {
			t.Fatalf("tool %d = %q, want %q", i, listed.Result.Tools[i].Name, n)
		}
	}
}
...
func writeSession(t *testing.T, home, dir, body string) {
	t.Helper()
	path := filepath.Join(home, ".theboringfloor", "projects", control.DirHash(dir), "session.json")
	if err := os.MkdirAll(filepath.Dir(path), 0755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(path, []byte(fmtJSON(body, dir)), 0600); err != nil {
		t.Fatal(err)
	}
}
func fmtJSON(template, dir string) string {
	return strings.ReplaceAll(template, "%q", strconv.Quote(dir))
}
