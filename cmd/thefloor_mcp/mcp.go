...
func serve(in io.Reader, out io.Writer, office *officeClient) error {
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal(scanner.Bytes(), &request); err != nil {
			if err := encoder.Encode(rpcResponse{JSONRPC: "2.0", ID: nil, Error: &rpcError{-32700, "parse error"}}); err != nil {
				return err
			}
			continue
		}
...
func handleRequest(request rpcRequest, id interface{}, office *officeClient) rpcResponse {
	if request.JSONRPC != "2.0" {
		return rpcResponse{JSONRPC: "2.0", ID: id, Error: &rpcError{-32600, "invalid JSON-RPC version"}}
	}
	...
	case "tools/call":
		...
		if err := json.Unmarshal(request.Params, &p); err != nil {
			return rpcResponse{JSONRPC: "2.0", ID: id, Error: &rpcError{-32602, "invalid tools/call params"}}
		}
