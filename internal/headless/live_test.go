...
// gated behind THEFLOOR_LIVE_CHROME=1 AND a successful Available
...
func liveServer(t *testing.T) *httptest.Server {
	t.Helper()
	if !config.EnvBool("LIVE_CHROME") {
		t.Skip("set THEFLOOR_LIVE_CHROME=1 to run the live engine tests")
	}
	path, ok := Available()
	if !ok {
