...
func (m *Model) applyEvent(ev state.Event) tea.Cmd {
	return tea.Batch(m.pagerKick(ev), m.applyMedia(ev), m.applyEventCore(ev), m.applyBrowserOpen(ev), m.applyRecentMessages(ev), m.applyPlanTools(ev), m.applyControl(ev), m.bypassLatchKick(ev), m.applyControlMutations(ev))
}
